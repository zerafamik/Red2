...
int main(void) {
    ...
    unsigned queued_cases=0,fallback_cases=0;
    const unsigned factors[4]={0,1,2,4};
    const size_t segments[3]={4096,5000,8192};
    for(unsigned k=0;k<24;k++) {
        uint64_t n=bound-(uint64_t)k*137u;
        size_t segment=segments[(k/4u)%3u];
        unsigned factor=factors[k%4u];
        int threads=k<12?1:3;
        check c={n,prime,0,0};red2_v34_stats stats;
        red2_v33_result r=red2_v34_sieve(n,threads,segment,19,70000,163,4,1,verify_segment,&c,factor,&stats);
        uint64_t y=(uint64_t)cbrt((double)n);
        while((y+1u)*(y+1u)*(y+1u)<=n)++y;
        while(y*y*y>n)--y;
        uint64_t expected_visits=0,ns=(n/30u+1u+segment-1u)/segment;
        for(uint64_t q=y+1u;q<=n/q;q++)if(prime[q])expected_visits+=ns-(q*q/30u)/segment;
        if(c.bad || c.blocks!=n/30u+1u || r.prime_count!=prefix[n] ||
           stats.direct_row_visits+stats.scheduled_row_visits>expected_visits ||
           (!stats.scheduled_rows && stats.direct_row_visits!=expected_visits) ||
           (stats.scheduled_rows && (!stats.scheduled_row_visits || !stats.calendar_bytes_per_worker))) {
            fprintf(stderr,"FAIL calendar factor=%u segment=%zu N=%" PRIu64 "\n",factor,segment,n);return 1;
        }
        queued_cases+=stats.scheduled_rows!=0;fallback_cases+=stats.memory_fallback!=0;
    }
#ifdef RED2_CALENDAR_MAX_BYTES
    if(RED2_CALENDAR_MAX_BYTES==64u && !fallback_cases)return 1;
#else
    if(!queued_cases)return 1;
#endif
    printf("PASS: %u original plus 24 calendar-specific full-bitmap cases through %" PRIu64 "; queued_cases=%u memory_fallback_cases=%u.\n",size,bound,queued_cases,fallback_cases);
