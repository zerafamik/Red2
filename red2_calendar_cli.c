...
int main(int argc,char **argv) {
    ...
    size_t segment=512u*1024u;unsigned multiplier=1,schedule_factor=2;
    for(int i=1;i<argc;i++) {
        const char *arg=argv[i];
        if(!strcmp(arg,"--version")){puts("RED2 v0.34 Product Calendar (experimental full sieve)");return 0;}
        if(!strcmp(arg,"--help")) {
            puts("RED2 full sieve: --limit N --threads T --segment-kib K --frontier-multiplier M --layer-max P --plate-batch B --schedule-factor F --list\n"
                 "Defaults: N=1e11, T=12, K=512, M=1, P=163, B=4, F=2 (0 disables calendar). Range: 0<=N<=1e13 (64-bit build).\n"
                 "--list emits ordered primes, uses one thread, and reports timing to stderr.\n"
                 "For a controlled comparison run bash compare_calendar.sh with the primesieve path.");return 0;
        }
        ...
        else if(!strcmp(arg,"--schedule-factor"))schedule_factor=(unsigned)number(value,64);
    ...
    red2_v34_stats stats;
    red2_v33_result r=red2_v34_sieve(n,threads,segment,19,70000,layer,batch,multiplier,list?emit:NULL,NULL,schedule_factor,&stats);
    ...
    fprintf(f,"{\"engine\":\"RED2_v0.34_product_calendar_full_sieve\",\"limit\":%" PRIu64 ",\"prime_count\":%" PRIu64
       ...
       "\"layer_max\":%d,\"plate_batch\":%d,\"dense_tile_bytes\":%zu,\"tasks\":%d,\"prepare_seconds\":%.9f,\"work_seconds\":%.9f,\"schedule_factor\":%u,\"direct_rows\":%zu,\"scheduled_rows\":%zu,\"calendar_bytes_per_worker\":%zu,\"ring_slots\":%u,\"max_cofactor_gap\":%u,\"memory_fallback\":%d,\"direct_row_visits\":%" PRIu64 ",\"scheduled_row_visits\":%" PRIu64 ",\"checkpoint\":\"%s\"}\n",
       n,r.prime_count,r.threads_used,r.segment_bytes,r.marking_frontier,r.aux_limit,r.residual_semiprime_count,
       r.estimated_peak_bytes,total,r.layer_max,r.plate_batch,r.dense_tile_bytes,r.task_count,r.prepare_seconds,r.work_seconds,stats.schedule_factor,stats.direct_rows,stats.scheduled_rows,stats.calendar_bytes_per_worker,stats.ring_slots,stats.max_cofactor_gap,stats.memory_fallback,stats.direct_row_visits,stats.scheduled_row_visits,expected==UINT64_MAX?"not_available":"pass");
    return ferror(f)?2:0;
}
